...
class WFunc:
    ...
    def get(self, phase, x, y, velocity):
        """x between 0 and 1"""
        angles = {}
        for j in self.pfn.keys():
            if phase:
                v = self.pfn[j].get(x,y)
                angles[j] = v
            else:
                angles[j] = self.afn[j].get(x,y)
        self.apply_velocity(angles, velocity, phase, x, y)
        return angles
    ...
    def apply_velocity(self, angles, velocity, phase, x, y):
        pass
        ...
        d = (x * 2 - 1) * v
        if phase:
            angles['l_f_hip_joint'] -= d

            angles['l_b_hip_joint'] -= d
            angles['r_f_hip_joint'] -= d

            angles['r_b_hip_joint'] -= d
        else:
            angles['l_f_hip_joint'] += d
...
class Walker:
    """Class for making Insect_bot walk"""
    def __init__(self, darwin):
        ...
        # self.ready_pos=get_walkk_angles(10)
        self.ready_pos = self.func.get(True, 0, 0, [0, 0, 0])
    ...
    def _do_walk(self):
        ...
        while (not rospy.is_shutdown() and
            ...
            x = float(i) / n
            y= float(w) / n
            angles = func.get(p, x, y, self.current_velocity)
            self.update_velocity(self.velocity, n)
