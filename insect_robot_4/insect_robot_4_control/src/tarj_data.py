...
def forward_gait():
    ...
    # Standard cycloid equation
    x, y, z = np.zeros(step_num / 2), np.zeros(step_num / 2), np.zeros(step_num / 2)
    E_long = v_E1 # Longitudinal step size, determining line speed
    E_lateral = v_E2 # Horizontal step size, determining line speed
    h = 0.1 # Step height
...
def turn_gait(direction):
    ...
    x1 = [0.60, 0.55, 0.60, 0.60, 0.60, 0.60, 0.60, 0.60 ]
    x2 = [0.60, 0.60, 0.60, 0.55, 0.60, 0.60, 0.60, 0.60 ]
    x3 = [0.60, 0.60, 0.60, 0.60, 0.60, 0.55, 0.60, 0.60 ]
    x4 = [0.60, 0.60, 0.60, 0.60, 0.60, 0.60, 0.60, 0.55 ]
...
def interpolate_method(y):
    ...
    #pl.plot(x,y,"ro")
    for kind in ["slinear"]:#Interpolation method
        #"nearest","zero"Step interpolation
        #slinear Linear interpolation
        #"quadratic","cubic" Interpolation for 2nd and 3rd order B-spline curves
